import{j as t}from"./jsx-runtime-BTJTZTIL.js";import{c as v}from"./index-DUmpqB7p.js";import{r}from"./index-CJ_LmFaV.js";import{C as W}from"./index-DHAmGKAZ.js";import{C as X}from"./index-Y3TRpJ-L.js";import{T as N}from"./Typography-DmSN6cv6.js";import{S as Q}from"./Sun-CjKF0Bsh.js";import{e as B}from"./componentPropsProcessors-UJEZy8aQ.js";import{T as j}from"./TruncateString-C-ApBzyA.js";import{B as E}from"./ButtonOutline-D517yaHT.js";import"./index-ChsGqxH_.js";import"./_commonjsHelpers-D6-XlEtG.js";import"./constants-BAH7I9kc.js";import"./useIsomorphicLayoutEffect-BAXQmlEq.js";import"./isBrowser-C2N6XvuH.js";import"./Tooltip-DMNQxdBh.js";import"./PopoverPrivate-TrV_4kB1.js";import"./index-Bxu_udwM.js";import"./index-DdXvKv-Q.js";import"./index-JZGyZbA2.js";import"./utils-D4P0b1iJ.js";import"./Counter-dO15w468.js";const Y=`# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

## 0.8.24 (2025-05-16)

### Only dependencies have been changed

- [@snack-uikit/icons@0.26.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.9.5](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.19](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.8.7](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.8.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.8.23 (2025-05-12)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.9.4](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)

## 0.8.22 (2025-03-19)

### Only dependencies have been changed

- [@snack-uikit/icons@0.26.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.18](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.21 (2025-03-17)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.17](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.20 (2025-03-17)

### Only dependencies have been changed

- [@snack-uikit/icons@0.26.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.9.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.16](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.8.6](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.8.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.8.19 (2025-03-14)

### Only dependencies have been changed

- [@snack-uikit/icons@0.26.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.15](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.18 (2025-03-13)

### Only dependencies have been changed

- [@snack-uikit/icons@0.25.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.14](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.17 (2025-03-05)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.13](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.16 (2025-03-04)

### Only dependencies have been changed

- [@snack-uikit/icons@0.25.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.12](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.15 (2025-03-04)

### Only dependencies have been changed

- [@snack-uikit/icons@0.25.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.11](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.14 (2025-03-04)

### Only dependencies have been changed

- [@snack-uikit/icons@0.24.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.9.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.10](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.8.5](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.8.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.8.13 (2025-02-10)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.9](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.12 (2025-02-06)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.8](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.11 (2025-02-04)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.7](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.10 (2025-01-23)

### Only dependencies have been changed

- [@snack-uikit/icons@0.24.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.9.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.6](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.8.4](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.7.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.8.9 (2025-01-20)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.5](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.8 (2024-12-26)

### Bug Fixes

- **FF-5734:** eslint fixes ([5ce6f1f](https://github.com/cloud-ru-tech/snack-uikit/commit/5ce6f1f22d3ac3dd367cbdd184e5873e67837c8b))

## 0.8.7 (2024-12-25)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.6.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.8.6 (2024-12-17)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.9.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)

## 0.8.5 (2024-12-12)

### Only dependencies have been changed

- [@snack-uikit/icons@0.24.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.8.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.6.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.8.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.6.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.8.4 (2024-11-29)

### Only dependencies have been changed

- [@snack-uikit/typography@0.8.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)

## 0.8.3 (2024-11-14)

### Dependencies

- **FF-5678:** up deps classnames/uncontrollable ([8f0d645](https://github.com/cloud-ru-tech/snack-uikit/commit/8f0d645fc7eb8eaf95660cd0ae7d4b550821059b))

## 0.8.2 (2024-11-12)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.8.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)

## 0.8.1 (2024-10-31)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.7.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)

# 0.8.0 (2024-10-28)

### Features

- **FF-5782:** sass use/forward approach ([3e53869](https://github.com/cloud-ru-tech/snack-uikit/commit/3e53869ace864a7718e434b7f410c15dbd911cd5))

# 0.7.0 (2024-10-28)

### Features

- **FF-5502:** add cjs build ([ffada28](https://github.com/cloud-ru-tech/snack-uikit/commit/ffada28bfdc37ea760eb1c8759342e680bdf8dd6))

## 0.6.14 (2024-09-26)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.24](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.13 (2024-09-24)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.23](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.12 (2024-09-23)

### Only dependencies have been changed

- [@snack-uikit/icons@0.22.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.5.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.22](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.6.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.4.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.6.11 (2024-09-06)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.21](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.10 (2024-07-31)

### Bug Fixes

- **FF-3788:** updated typescript to v5 ([89e3338](https://github.com/cloud-ru-tech/snack-uikit/commit/89e3338a3ef4df24e4c20d9a9c8cd16e7934ba63))

## 0.6.9 (2024-07-19)

### Only dependencies have been changed

- [@snack-uikit/icons@0.22.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.20](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.8 (2024-06-27)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.19](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.7 (2024-06-20)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.18](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.6 (2024-05-31)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.17](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.5 (2024-05-30)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.16](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.4 (2024-05-23)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.15](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.3 (2024-05-08)

### Only dependencies have been changed

- [@snack-uikit/icons@0.21.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.14](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.6.2 (2024-04-25)

### Bug Fixes

- **PDS-0000:** set default button type as button ([4ec64de](https://github.com/cloud-ru-tech/snack-uikit/commit/4ec64de508826df4a1e82fd8c75448d9919e563f))

## 0.6.1 (2024-04-23)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.5.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.13](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.6.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.3.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

# 0.6.0 (2024-04-04)

### BREAKING CHANGES

- **FF-4649:** add stepper jsx and validation function to stepperApi object ([47df0d2](https://github.com/cloud-ru-tech/snack-uikit/commit/47df0d2e02370d05e604f64d01441ec09e74a794))

## 0.5.9 (2024-03-26)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.12](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.8 (2024-03-22)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.11](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.7 (2024-02-29)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.10](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.6 (2024-02-26)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.9](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.5 (2024-02-20)

### Bug Fixes

- **FF-4297:** apply system-layer tokens ([7c8fd4b](https://github.com/cloud-ru-tech/snack-uikit/commit/7c8fd4b5334360b2fc31da92973b6835ffa287af))

## 0.5.4 (2024-02-14)

### Bug Fixes

- **FF-0000:** optional props ([efa7f9c](https://github.com/cloud-ru-tech/snack-uikit/commit/efa7f9cb22ffc26a27ee1eb8ccbdf4b0facea628))

## 0.5.3 (2024-02-05)

### Only dependencies have been changed

- [@snack-uikit/icons@0.20.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.7](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.2 (2024-02-01)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.6](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.5.1 (2024-01-23)

### Only dependencies have been changed

- [@snack-uikit/icons@0.20.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.5](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

# 0.5.0 (2023-12-28)

### Features

- **FF-3960:** add step description, change step layout ([bbda81d](https://github.com/cloud-ru-tech/snack-uikit/commit/bbda81d731bcde8a00577ec63d089d120808a90a))

## 0.4.3 (2023-12-25)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.4.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.4.2 (2023-12-18)

### Only dependencies have been changed

- [@snack-uikit/icons@0.19.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.4.1 (2023-12-14)

### Only dependencies have been changed

- [@snack-uikit/icons@0.19.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.4.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

# 0.4.0 (2023-12-14)

### BREAKING CHANGES

- **FF-3729:** replace enum with unions ([910db4a](https://github.com/cloud-ru-tech/snack-uikit/commit/910db4aa8231ccbc58e538e5c5c1f461b1dec275))

## 0.3.1 (2023-12-06)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.4.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.3.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.5.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.1.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

# 0.3.0 (2023-12-06)

### Bug Fixes

- **FF-3026:** replaced loader with sun in stepper ([5310087](https://github.com/cloud-ru-tech/snack-uikit/commit/5310087b0d40ec9bfaf25f64797c98fc1c57427c))

### Features

- **FF-2659:** add stepper package ([84da231](https://github.com/cloud-ru-tech/snack-uikit/commit/84da2319642ef7a8a31bb2020d0c914bfd61ea04))
- **FF-875:** add utils for dependencies graph ([53cd7be](https://github.com/cloud-ru-tech/snack-uikit/commit/53cd7be638f01e573cb52b2417a39f4df4f6089b))

### BREAKING CHANGES

- **FF-3823:** move to snack-uikit scope ([ac9e30d](https://github.com/cloud-ru-tech/snack-uikit/commit/ac9e30d574d529b2bf4f5184b70c511c981a6032))

## 0.2.18 (2023-11-16)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.2.16](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.17 (2023-11-09)

### Only dependencies have been changed

- [@snack-uikit/icons@0.18.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.15](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.16 (2023-11-08)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.3.4](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.14](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.4.4](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@2.0.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.2.15 (2023-10-31)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.3.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.13](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.4.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@2.0.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.2.14 (2023-10-28)

### Only dependencies have been changed

- [@snack-uikit/icons@0.19.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/loaders@0.4.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.3.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.5.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@3.0.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.2.13 (2023-10-26)

### Only dependencies have been changed

- [@snack-uikit/icons@0.18.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.12](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.12 (2023-10-25)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.3.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.11](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.4.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)
- [@snack-uikit/utils@1.3.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/utils/CHANGELOG.md)

## 0.2.11 (2023-10-16)

### Only dependencies have been changed

- [@snack-uikit/icons@0.17.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.10](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.10 (2023-10-13)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.2.9](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.4.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)

## 0.2.9 (2023-10-12)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.3.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.8](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.8 (2023-10-12)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.2.7](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.7 (2023-10-11)

### Only dependencies have been changed

- [@snack-uikit/icons@0.16.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.6](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.6 (2023-10-06)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.3.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.5](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)
- [@snack-uikit/typography@0.4.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)

## 0.2.5 (2023-09-28)

### Only dependencies have been changed

- [@snack-uikit/icons@0.15.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.4](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.4 (2023-09-25)

### Only dependencies have been changed

- [@snack-uikit/icons@0.14.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.3](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.3 (2023-09-21)

### Bug Fixes

- **FF-3026:** replaced loader with sun in stepper ([5310087](https://github.com/cloud-ru-tech/snack-uikit/commit/5310087b0d40ec9bfaf25f64797c98fc1c57427c))

## 0.2.2 (2023-09-14)

### Only dependencies have been changed

- [@snack-uikit/icons@0.13.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.2](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.2.1 (2023-09-12)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.2.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

# 0.2.0 (2023-09-08)

### Features

- **FF-875:** add utils for dependencies graph ([53cd7be](https://github.com/cloud-ru-tech/snack-uikit/commit/53cd7be638f01e573cb52b2417a39f4df4f6089b))

## 0.1.8 (2023-08-31)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.1.9](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.1.7 (2023-08-30)

### Only dependencies have been changed

- [@snack-uikit/typography@0.3.1](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)

## 0.1.6 (2023-08-29)

### Only dependencies have been changed

- [@snack-uikit/typography@0.3.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/typography/CHANGELOG.md)

## 0.1.5 (2023-08-24)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.1.8](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.1.4 (2023-08-24)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.1.7](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.1.3 (2023-08-22)

### Only dependencies have been changed

- [@snack-uikit/loaders@0.2.9](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/loaders/CHANGELOG.md)

## 0.1.2 (2023-08-17)

### Only dependencies have been changed

- [@snack-uikit/icons@0.12.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/icons/CHANGELOG.md)
- [@snack-uikit/truncate-string@0.2.0](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

## 0.1.1 (2023-08-17)

### Only dependencies have been changed

- [@snack-uikit/truncate-string@0.1.6](https://github.com/cloud-ru-tech/snack-uikit/blob/master/packages/truncate-string/CHANGELOG.md)

# 0.1.0 (2023-08-17)

### Features

- **FF-2659:** add stepper package ([84da231](https://github.com/cloud-ru-tech/snack-uikit/commit/84da2319642ef7a8a31bb2020d0c914bfd61ea04))

## CHANGELOG

### v0.0.0

- Initial version
`,Z="@snack-uikit/stepper",tt="0.8.24",T={name:Z,version:tt},nt=`# Stepper

Компонент отображения состояния пошагового процесса.

## Installation
\`npm i @snack-uikit/stepper\`

[Changelog](./CHANGELOG.md)

#### Использование

Компонент принимает массив шагов. В проп children принимает render-колбек, в который отдает сам компонент, методы управления и поля состояния.
При вызове метода \`goNext\` вызывается валидация текущего шага. Функция валидации принимает индекс текущего шага и должна вернуть результат валидации в Promise<boolean>. При успешной валдиции компонент переходит на следующий шаг, а при ошибке остается на текущем.
При вызове метода \`goPrev\` компонент переходит на предыдущий шаг.


\`\`\`tsx
const steps = [
  { title: 'Step #1' },
  { title: 'Step #2' },
  { title: 'Step #3' },
  { title: 'Step #4' },
];

// ...

return (
  <Stepper steps={steps} validator={validatorFunction}>
    {({
        stepper, // JSX элемент компонента
        isCompleted, // состояние завершенности всей цепочки
        goNext, // функция перехода к след. шагу
        goPrev, // функция перехода к предыдущему шагу
        setValidator, // функция установки валидатора
      }) => (
      <>
        {stepper}

        <button onClick={() => goPrev()}>go back</button>
        <button onClick={() => goNext()}>go next</button>
      </>
    )}
  </Stepper>
)
\`\`\`
[//]: DOCUMENTATION_SECTION_START
[//]: THIS_SECTION_IS_AUTOGENERATED_PLEASE_DONT_EDIT_IT
## Stepper
### Props
| name | type | default value | description |
|------|------|---------------|-------------|
| children* | \`({stepper, ...api}) => ReactElement\` | - | Render function. Принимает аргументы: \`stepper\` - JSX-элемент степпера, \`goNext(stepIndex?: number)\` - перейти на след. шаг, \`goPrev(stepIndex?: number)\` - перейти на пред. шаг, \`resetValidation\` - сбросить состояние валидации для текущего шага, \`setValidator\` переопределяет функцию-валидатор, которая принимает в параметры индекс текущего шага и индекс нового, \`isCompleted\` - окончен ли процесс, \`currentStepIndex\` - индекс текущего шага, \`stepCount\` - кол-во шагов. |
| steps* | \`StepData[]\` | - | Массив шагов |
| defaultCurrentStepIndex | \`number\` | - | Индекс текущего шага по-дефолту |
| validator | \`StepsValidator\` | - | Валидатор шагов. Выполняется при смене шага. Принимает первым аргументом индекс текущего, вторым - индекс нового шага. |
| className | \`string\` | - | CSS-класс |
| onChangeCurrentStep | \`(newValue: number, prevValue: number) => void\` | - | Колбек смены текущего степа |
| onCompleteChange | \`(isCompleted: boolean) => void\` | - | Колбек изменения завершенности |


[//]: DOCUMENTATION_SECTION_END
`,a={Completed:"completed",Current:"current",Loading:"loading",Waiting:"waiting",Rejected:"rejected"},w=n=>e=>n&&e?`${e}${n}`:void 0,et="_icon_11lr3_1",ct={icon:et};function ut(n,e){switch(n){case a.Completed:return t.jsx(X,{});case a.Rejected:return t.jsx(W,{});case a.Loading:return t.jsx(Q,{size:"s"});default:return e}}function A({state:n,number:e,className:d}){const i=r.useMemo(()=>ut(n,e),[e,n]);return t.jsx("div",{"data-state":n,className:v(ct.icon,d),children:typeof i=="number"?t.jsx(N.SansLabelL,{children:i}):i})}try{A.displayName="Icon",A.__docgenInfo={description:"",displayName:"Icon",props:{state:{defaultValue:null,description:"",name:"state",required:!0,type:{name:"enum",value:[{value:'"loading"'},{value:'"current"'},{value:'"completed"'},{value:'"waiting"'},{value:'"rejected"'}]}},number:{defaultValue:null,description:"",name:"number",required:!0,type:{name:"number"}},className:{defaultValue:null,description:"",name:"className",required:!1,type:{name:"string"}}}}}catch{}const at="_tailLine_1y0ss_1",st="_tail_1y0ss_1",it="_title_1y0ss_19",rt="_description_1y0ss_24",ot="_step_1y0ss_29",dt="_track_1y0ss_40",lt="_statusContainer_1y0ss_46",kt="_content_1y0ss_75",pt="_icon_1y0ss_81",k={tailLine:at,tail:st,title:it,description:rt,step:ot,track:dt,statusContainer:lt,content:kt,icon:pt},ht=w("_element-tail"),mt=w("_element-step");function L({step:n,className:e,"data-test-id":d,hideTailLine:i,...o}){return t.jsxs("div",{className:v(k.step,e),"data-state":n.state,...B(o),children:[t.jsxs("div",{className:k.track,children:[t.jsx("button",{type:"button",className:k.statusContainer,onClick:n.onClick,disabled:!n.onClick,"data-test-id":mt(d),"data-state":n.state,children:t.jsx(A,{...n,className:k.icon})}),!i&&t.jsx("div",{className:k.tail,"data-completed":n.state===a.Completed||void 0,"data-test-id":ht(d),children:t.jsx("div",{className:k.tailLine})})]}),t.jsxs("div",{className:k.content,children:[t.jsx(N.SansBodyM,{className:k.title,tag:"div",children:t.jsx(j,{text:n.title})}),n.description&&t.jsx(N.SansBodyS,{className:k.description,tag:"div",children:t.jsx(j,{text:n.description,maxLines:2})})]})]})}try{L.displayName="Step",L.__docgenInfo={description:"",displayName:"Step",props:{"data-test-id":{defaultValue:null,description:"",name:"data-test-id",required:!1,type:{name:"string"}},hideTailLine:{defaultValue:null,description:"",name:"hideTailLine",required:!1,type:{name:"boolean"}},step:{defaultValue:null,description:"",name:"step",required:!0,type:{name:"StepViewData"}},className:{defaultValue:null,description:"",name:"className",required:!1,type:{name:"string"}}}}}catch{}const P=r.createContext({stepper:t.jsx(t.Fragment,{}),stepCount:0,isCompleted:!1,currentStepIndex:0,goNext(){},goPrev(){},resetValidation(){},setValidator(){}}),gt=()=>r.useContext(P),bt="_stepper_y1r4g_1",Gt={stepper:bt},Ct=async()=>!0;function O({children:n,steps:e,className:d,onChangeCurrentStep:i,onCompleteChange:o,defaultCurrentStepIndex:f=0,validator:H=Ct,...F}){const _=f===e.length-1,[g,p]=r.useState(_?a.Completed:a.Current),[u,q]=r.useState(f),[l,S]=r.useState(_),[h,$]=r.useState();r.useEffect(()=>{o==null||o(l)},[l,o]);const b=r.useCallback(c=>{q(s=>(s!==c&&(i==null||i(c,s)),c))},[i]),y=r.useCallback((c=u+1)=>{if(u>=e.length||l||c<=u)return;const s=Math.min(e.length-1,c);p(a.Loading),((h==null?void 0:h.value)||H)(u,s).catch(()=>!1).then(z=>{if(!z){p(a.Rejected);return}u===e.length-1?(p(a.Completed),S(!0)):(b(s),p(a.Current))})},[u,l,b,e.length,h==null?void 0:h.value,H]),G=r.useCallback((c=u-1)=>{u===0||c<0||c>u||c===u&&!l||(l&&S(!1),b(c),p(a.Current))},[u,l,b]),M=r.useMemo(()=>e.map((c,s)=>{const m=s+1;return s<u?{...c,number:m,state:a.Completed,onClick:()=>G(s)}:s===u?{...c,number:m,state:g,onClick:l?()=>G(s):void 0}:s-1===u?{...c,number:m,state:a.Waiting,onClick:()=>y()}:{...c,number:m,state:a.Waiting}}),[e,u,G,g,l,y]),J=r.useCallback(()=>{g===a.Rejected&&p(a.Current)},[g]),K=r.useCallback(c=>{$({value:c})},[]),U=t.jsx("div",{className:v(Gt.stepper,d),...B(F),children:M.map((c,s)=>t.jsx(L,{step:c,"data-test-id":F["data-test-id"],hideTailLine:s===e.length-1},c.title+s))}),x={goNext:y,goPrev:G,currentStepIndex:u,isCompleted:l,resetValidation:J,stepCount:e.length,stepper:U,setValidator:K};return t.jsx(P.Provider,{value:x,children:n(x)})}try{O.displayName="Stepper",O.__docgenInfo={description:"",displayName:"Stepper",props:{"data-test-id":{defaultValue:null,description:"",name:"data-test-id",required:!1,type:{name:"string"}},steps:{defaultValue:null,description:"Массив шагов",name:"steps",required:!0,type:{name:"StepData[]"}},defaultCurrentStepIndex:{defaultValue:{value:"0"},description:"Индекс текущего шага по-дефолту",name:"defaultCurrentStepIndex",required:!1,type:{name:"number"}},validator:{defaultValue:null,description:"Валидатор шагов. Выполняется при смене шага. Принимает первым аргументом индекс текущего, вторым - индекс нового шага.",name:"validator",required:!1,type:{name:"StepsValidator"}},className:{defaultValue:null,description:"CSS-класс",name:"className",required:!1,type:{name:"string"}},children:{defaultValue:null,description:"Render function. Принимает аргументы: `stepper` - JSX-элемент степпера,\n`goNext(stepIndex?: number)` - перейти на след. шаг, `goPrev(stepIndex?: number)` - перейти на пред. шаг, `resetValidation` - сбросить состояние валидации для текущего шага, `setValidator` переопределяет функцию-валидатор, которая принимает в параметры индекс текущего шага и индекс нового, `isCompleted` - окончен ли процесс, `currentStepIndex` - индекс текущего шага, `stepCount` - кол-во шагов.",name:"children",required:!0,type:{name:"({stepper, ...api}) => ReactElement"}},onChangeCurrentStep:{defaultValue:null,description:"Колбек смены текущего степа",name:"onChangeCurrentStep",required:!1,type:{name:"((newValue: number, prevValue: number) => void)"}},onCompleteChange:{defaultValue:null,description:"Колбек изменения завершенности",name:"onCompleteChange",required:!1,type:{name:"((isCompleted: boolean) => void)"}}}}}catch{}const Ot="_story_142nc_1",yt="_panel_142nc_9",R={story:Ot,panel:yt},Kt={title:"Components/Stepper",component:O};function Et(){const{goPrev:n,goNext:e,isCompleted:d,currentStepIndex:i,resetValidation:o}=gt();return t.jsxs(t.Fragment,{children:[t.jsxs("div",{children:[t.jsxs("div",{"data-test-id":"is-completed",children:["isCompleted: ",d?"yes":"no"]}),t.jsxs("div",{"data-test-id":"current-index",children:["current step index: ",i]})]}),t.jsxs("div",{className:R.panel,children:[t.jsx(E,{"data-test-id":"prev",size:"s",label:"go prev",onClick:()=>n()}),t.jsx(E,{"data-test-id":"reset-validation",size:"s",label:"reset validation",onClick:o}),t.jsx(E,{"data-test-id":"next",size:"s",label:"go next",onClick:()=>e()})]})]})}const Nt=({validationTimeout:n,isValid:e,...d})=>{const i=()=>new Promise(o=>setTimeout(()=>o(e),n));return t.jsx(O,{...d,validator:i,children:({stepper:o})=>t.jsxs(t.Fragment,{children:[t.jsx("div",{className:R.story,children:o}),t.jsx(Et,{})]})})},C={render:Nt,args:{steps:[{title:"First step",description:"First step long description two lines truncated"},{title:"Second step",description:"Second step long description two lines truncated"},{title:"Third step",description:"Third step long description two lines truncated"},{title:"Fourth step",description:"Fourth step long description two lines truncated"},{title:"Fifth step",description:"Fifth step long description two lines truncated"}],isValid:!0,validationTimeout:1e3},argTypes:{isValid:{name:"[stories] is current step valid",control:{type:"boolean"}},validationTimeout:{name:"[stories] validation timeout",control:{min:0,type:"number"}}},parameters:{readme:{sidebar:[`Latest version: ${T.version}`,nt,Y]},packageName:T.name,design:{name:"Figma",type:"figma",url:"https://www.figma.com/file/jtGxAPvFJOMir7V0eQFukN/Snack-UI-Kit-1.1.0?node-id=41%3A33355&mode=design"}}};var D,I,V;C.parameters={...C.parameters,docs:{...(D=C.parameters)==null?void 0:D.docs,source:{originalSource:`{
  render: Template,
  args: {
    steps: [{
      title: 'First step',
      description: 'First step long description two lines truncated'
    }, {
      title: 'Second step',
      description: 'Second step long description two lines truncated'
    }, {
      title: 'Third step',
      description: 'Third step long description two lines truncated'
    }, {
      title: 'Fourth step',
      description: 'Fourth step long description two lines truncated'
    }, {
      title: 'Fifth step',
      description: 'Fifth step long description two lines truncated'
    }],
    isValid: true,
    validationTimeout: 1000
  },
  argTypes: {
    isValid: {
      name: '[stories] is current step valid',
      control: {
        type: 'boolean'
      }
    },
    validationTimeout: {
      name: '[stories] validation timeout',
      control: {
        min: 0,
        type: 'number'
      }
    }
  },
  parameters: {
    readme: {
      sidebar: [\`Latest version: \${componentPackage.version}\`, componentReadme, componentChangelog]
    },
    packageName: componentPackage.name,
    design: {
      name: 'Figma',
      type: 'figma',
      url: 'https://www.figma.com/file/jtGxAPvFJOMir7V0eQFukN/Snack-UI-Kit-1.1.0?node-id=41%3A33355&mode=design'
    }
  }
}`,...(V=(I=C.parameters)==null?void 0:I.docs)==null?void 0:V.source}}};export{Kt as default,C as stepper};
